...
export async function onRequestPost(context) {
  try {
    ...
    await db.prepare('UPDATE bookings SET status = ? WHERE id = ?').bind(targetStatus, id).run();
    ...
    return new Response(
      html`
        <div
          class="booking-row status-${b.status}"
          id="booking-container-${b.id}"
          style="display: flex; align-items: center; justify-content: space-between; background: var(--surface-2); padding: var(--size-3); margin-bottom: var(--size-2); border-radius: var(--radius-2); border: var(--border-size-1) solid var(--surface-3);"
        >
          <div class="booking-info">
            ...
            <span
              ...
              style="padding: var(--size-1) var(--size-2); border-radius: var(--radius-1); font-size: var(--font-size-0); font-weight: bold;"
              >${statusLabel}</span
            >
          </div>
