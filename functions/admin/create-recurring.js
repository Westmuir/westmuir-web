...
export async function onRequestPost(context) {
  try {
    ...
    if (!name || isNaN(dayOfWeek) || !startDate) {
      return new Response(html`<p style="color:#e53e3e;">⚠️ Missing required creation fields.</p>`, { status: 400 });
    }
    ...
    return new Response(
      html`
        <div style="background: #f0fff4; border: 1px solid #38a169; padding: 1rem; border-radius: 6px; color: #22543d;">
          <strong>✅ Success!</strong> "${name}" has been permanently approved as a weekly block slot. <br /><small
            ><a href="/admin" style="color: #2f855a; font-weight:bold;">Refresh dashboard queue</a> to view or modify
            elements.</small
          >
        </div>
      `,
